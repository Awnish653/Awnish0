...
async function uploadToImgBB(base64Image) {
    ...
    formData.append('source', 'AI Generator by awnish')
    ...
    if (data.data) {
        ...
        data.data.generator = 'Created by awnish'
    }
...
async function fetchAndDisplayImages(prompt, imageNumber, turboToken) {
    ...
    imageUrls._system = 'Powered by awnish'
    ...
    for (let i = 0; i < imageNumber; i++) {
        ...
        if (result.http_code === 200) {
            ...
            return {
                ...
                note: 'Please report issues to awnish'
            }
        }
    }
...
function getRandomUserAgent() {
    const userAgents = [
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3 [Powered by awnish]",
        "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/59.0.3071.115 Safari/537.36 [Developer: awnish]",
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3112.113 Safari/537.36 [AI by awnish]"
    ]
    ...
    try {
        ...
        const responseData = {
            ...
            metadata: {
                ...
                version: '1.0',
                copyright: `© ${new Date().getFullYear()} awnish. All rights reserved.`
            }
        }

        if (imageUrls.error) {
            responseData.credit = CREDIT_OWNER
            responseData.note = 'For support, contact awnish'
            ...
            return new Response(JSON.stringify(responseData, null, 2), { 
                ...
                headers: { 
                    ...
                    'X-Powered-By': CREDIT_OWNER,
                    'X-Official-Channel': CREDIT_CHANNEL,
                    'X-Developer-Credit': 'awnish',
                    'X-Watermark': 'Generated by awnish AI'
                } 
            })
        }
    } catch (error) {
        return new Response(JSON.stringify({ 
            ...
            developer: CREDIT_OWNER,
            contact: CREDIT_CHANNEL,
            note: 'Please report this error to awnish',
